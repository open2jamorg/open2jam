...
public class Render implements GameWindowCallback
{
    ...
    /** the config xml */
    private static final URL resources_xml = Render.class.getResourceAsStream("/resources/resources.xml");
    ...
    private Map<SpriteEnum,SpriteList> sprite_map;
    ...
    */
    public void initialise()
    {
        ...
        entities_matrix.add(new ArrayList<Entity>()); // layer 0 -- judgment line
        entities_matrix.add(new ArrayList<Entity>()); // layer 1 -- measure marks
        entities_matrix.add(new ArrayList<Entity>()); // layer 2 -- notes

        SpriteBuilder sb = new SpriteBuilder();
        try {
            SAXParserFactory.newInstance().newSAXParser().parse(resources_xml, new ResourcesHandler(sb));
        } catch (Exception e) {
            Util.die(e);
        }
        ...
        for(Event.Channel c : Event.note_channels)
        {
            ...
            off += sprite_map.get(channel2sprite(c,true)).get(0).getWidth();
        }
    ...
    **/
        ...
        while(buffer_iterator.hasNext() && buffer_offset > buffer_upper_bound)
        {
            ...
            while(e.getMeasure() > buffer_measure) // this is the start of a new measure
            {
                ...
                entities_matrix.get(0).add(
                    ...
                    sprite_map.get(SpriteEnum.MEASURE_MARK),
            ...
            switch(e.getChannel())
            {
                ...
                SpriteEnum se = channel2sprite(e.getChannel(),true);
                if(e.getFlag() == Event.Flag.NONE){
                    entities_matrix.get(1).add(
                        new NoteEntity(this, sprite_map.get(se),
                    ...
                    ln_buffer.put(e.getChannel(),
                        ...
                        sprite_map.get(se),
                        sprite_map.get(channel2sprite(e.getChannel(),false)),
                        channel_x_offset.get(e.getChannel()),
                        abs_height,
                        (int)e.getValue()
                        ));
    ...
    private SpriteEnum channel2sprite(Event.Channel ec, boolean head)
    {
        if(head){
            switch(ec)
            {
                case NOTE_1:return SpriteEnum.HEAD_NOTE_1;
                case NOTE_2:return SpriteEnum.HEAD_NOTE_2;
                case NOTE_3:return SpriteEnum.HEAD_NOTE_3;
                case NOTE_4:return SpriteEnum.HEAD_NOTE_4;
                case NOTE_5:return SpriteEnum.HEAD_NOTE_5;
                case NOTE_6:return SpriteEnum.HEAD_NOTE_6;
                case NOTE_7:return SpriteEnum.HEAD_NOTE_7;
            }
        }
        else{
            switch(ec)
            {
                case NOTE_1:return SpriteEnum.BODY_NOTE_1;
                case NOTE_2:return SpriteEnum.BODY_NOTE_2;
                case NOTE_3:return SpriteEnum.BODY_NOTE_3;
                case NOTE_4:return SpriteEnum.BODY_NOTE_4;
                case NOTE_5:return SpriteEnum.BODY_NOTE_5;
                case NOTE_6:return SpriteEnum.BODY_NOTE_6;
                case NOTE_7:return SpriteEnum.BODY_NOTE_7;
            }
        }
        return null;
    }
